...
const mockPersons: Person[] = [
  { id: 1, name: "Ahmet Yılmaz", type: "Gozlem", risk: "Yuksek", status: "Aktif", lastUpdate: "2026-04-17", identity: "TR-001-247" },
  { id: 2, name: "Elif Kaya", type: "Kaynak", risk: "Orta", status: "Aktif", lastUpdate: "2026-04-16", identity: "TR-002-189" },
  { id: 3, name: "Mehmet Demir", type: "Hedef", risk: "Kritik", status: "Gizli", lastUpdate: "2026-04-15", identity: "TR-003-156" },
  { id: 4, name: "Ayşe Çelik", type: "Kaynak", risk: "Dusuk", status: "Aktif", lastUpdate: "2026-04-14", identity: "TR-004-123" },
  { id: 5, name: "Mustafa Koç", type: "Gozlem", risk: "Orta", status: "Arsiv", lastUpdate: "2026-04-13", identity: "TR-005-098" },
  { id: 6, name: "Fatma Şahin", type: "Hedef", risk: "Kritik", status: "Gizli", lastUpdate: "2026-04-12", identity: "TR-006-087" },
  { id: 7, name: "Ali Öztürk", type: "Kaynak", risk: "Dusuk", status: "Aktif", lastUpdate: "2026-04-11", identity: "TR-007-076" },
  { id: 8, name: "Zeynep Ak", type: "Gozlem", risk: "Yuksek", status: "Aktif", lastUpdate: "2026-04-10", identity: "TR-008-065" },
]
...
const riskLabels: Record<string, { label: string; color: string; bg: string }> = {
  ...
  Yuksek: { label: "Yüksek", color: "text-orange-500", bg: "bg-orange-500/10" },
                ...
                {filteredPersons.map((person) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground">{person.identity}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-[10px] uppercase ${typeLabels[person.type].color}`}>
                        {typeLabels[person.type].label}
                      </span>
                      <span className={`text-[10px] uppercase ${riskLabels[person.risk].color}`}>
              ...
              <Link key={person.id} to={`/app/person/${person.id}`}>
                <Card className="hover:bg-muted transition-colors">
                  <CardContent className="pt-4">
                    ...
                    <p className="text-xs font-medium truncate text-center">{person.name}</p>
                    <p className="text-[10px] text-muted-foreground text-center truncate mt-0.5">
                      {person.identity}
                    </p>
                    <div className="flex justify-center gap-2 mt-2">
                      <span className={`text-[9px] uppercase ${typeLabels[person.type].color}`}>
                        {typeLabels[person.type].label}
                      </span>
                      <span className={`text-[9px] uppercase ${riskLabels[person.risk].color}`}>
                        {riskLabels[person.risk].label}
                      </span>
                    </div>
                  </CardContent>
        ...
        {filteredPersons.length === 0 && (
          <div className="text-center py-12 space-y-2">
            <svg className="w-12 h-12 mx-auto text-muted-foreground/30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeWidth="1" d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z" />
